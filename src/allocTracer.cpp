...
void AllocTracer::trapHandler(int signo, siginfo_t *siginfo, void *ucontext) {
  ...
  if (_in_new_tlab.covers(frame.pc())) {
    // send_allocation_in_new_tlab(Klass* klass, HeapWord* obj, size_t
    // tlab_size, size_t alloc_size, Thread* thread)
    // send_allocation_in_new_tlab_event(KlassHandle klass, size_t tlab_size,
    // size_t alloc_size)
    ...
    // send_allocation_outside_tlab(Klass* klass, HeapWord* obj, size_t
    // alloc_size, Thread* thread)
...
Error AllocTracer::check(Arguments &args) {
  ...
  if ((ne = libjvm->findSymbolByPrefix(
           "_ZN11AllocTracer27send_allocation_in_new_tlab")) != NULL &&
      (oe = libjvm->findSymbolByPrefix(
           "_ZN11AllocTracer28send_allocation_outside_tlab")) != NULL) {
    _trap_kind = 1; // JDK 10+
  } else if ((ne = libjvm->findSymbolByPrefix(
                  "_ZN11AllocTracer33send_allocation_in_new_tlab_"
                  "eventE11KlassHandleP8HeapWord")) != NULL &&
             (oe = libjvm->findSymbolByPrefix(
                  "_ZN11AllocTracer34send_allocation_outside_tlab_"
                  "eventE11KlassHandleP8HeapWord")) != NULL) {
