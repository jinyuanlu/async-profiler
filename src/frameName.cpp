...
FrameName::FrameName(Arguments &args, int style, int epoch,
                     Mutex &thread_names_lock, ThreadMap &thread_names)
    : _class_names(), _include(), _exclude(), _str(), _style(style),
      _cache_epoch((unsigned char)epoch), _cache_max_age(args._mcache),
      _thread_names_lock(thread_names_lock), _thread_names(thread_names) {
...
FrameName::~FrameName() {
  if (_cache_max_age == 0) {
    ...
    // Remove stale methods from the cache, leave the fresh ones for the next
    // profiling session
...
void FrameName::buildFilter(std::vector<Matcher> &vector, const char *base,
                            int offset) {
  while (offset != 0) {
...
const char *FrameName::decodeNativeSymbol(const char *name) {
  const char *lib_name = (_style & STYLE_LIB_NAMES)
...
void FrameName::javaMethodName(jmethodID method) {
  ...
  if ((err = jvmti->GetMethodName(method, &method_name, &method_sig, NULL)) ==
          0 &&
...
const char *FrameName::name(ASGCT_CallFrame &frame, bool for_matching) {
  ...
  case BCI_ERROR:
    return _str.assign("[")
        .append((const char *)frame.method_id)
        .append("]")
        .c_str();
  ...
  default: {
    ...
    _cache.insert(
        it, JMethodCache::value_type(frame.method_id,
...
FrameTypeId FrameName::type(ASGCT_CallFrame &frame) {
  ...
  case BCI_NATIVE_FRAME: {
    ...
    if ((name[0] == '_' && name[1] == 'Z') ||
      ...
      return len > 4 && strcmp(name + len - 4, "_[k]") == 0 ? FRAME_KERNEL
                                                            : FRAME_NATIVE;
    }
  }
