...
void LockTracer::initialize() {
  ...
  jmethodID register_natives =
      env->GetStaticMethodID(_UnsafeClass, "registerNatives", "()V");
  jniNativeInterface *jni_functions;
  if (register_natives != NULL &&
      jvmti->GetJNIFunctionTable(&jni_functions) == 0) {
    ...
    // Trace Unsafe.registerNatives() to find the original address of
    // Unsafe.park() native
  ...
  _LockSupport = (jclass)env->NewGlobalRef(
      env->FindClass("java/util/concurrent/locks/LockSupport"));
  _getBlocker = env->GetStaticMethodID(
      _LockSupport, "getBlocker", "(Ljava/lang/Thread;)Ljava/lang/Object;");
...
void JNICALL LockTracer::MonitorContendedEntered(jvmtiEnv *jvmti, JNIEnv *env,
  ...
  if (_enabled && entered_time - enter_time >= _threshold &&
      enter_time >= _start_time) {
    char *lock_name = getLockName(jvmti, env, object);
    recordContendedLock(LOCK_SAMPLE, enter_time, entered_time, lock_name,
                        object, 0);
...
char *LockTracer::getLockName(jvmtiEnv *jvmti, JNIEnv *env, jobject lock) {
  char *class_name;
  if (jvmti->GetClassSignature(env->GetObjectClass(lock), &class_name, NULL) !=
      0) {
    return NULL;
  }
  return class_name;
...
bool LockTracer::isConcurrentLock(const char *lock_name) {
  ...
  // and Semaphore
  return strncmp(lock_name, "Ljava/util/concurrent/locks/ReentrantLock", 41) ==
             0 ||
         strncmp(lock_name,
...
void LockTracer::recordContendedLock(EventType event_type, u64 start_time,
                                     u64 end_time, const char *lock_name,
                                     jobject lock, jlong timeout) {
  LockEvent event;
  event._class_id = 0;
