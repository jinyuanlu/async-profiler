...
static u32 lookupClassId(jvmtiEnv *jvmti, jclass cls) {
  ...
  if (jvmti->GetClassSignature(cls, &class_name, NULL) == 0) {
    if (class_name[0] == 'L') {
      class_id = Profiler::instance()->classMap()->lookup(
          class_name + 1, strlen(class_name) - 2);
    } else {
...
public:
  ...
  void add(JNIEnv *jni, jobject object, jlong size, u64 trace) {
    ...
    if (_lock.tryLock()) {
      u32 start =
          (((uintptr_t)object >> 4) * 31 + ((uintptr_t)jni >> 4) + trace) &
          (MAX_REFS - 1);
      u32 i = start;
      do {
        jweak w = _refs[i];
    ...
    for (u32 i = 0; i < MAX_REFS; i++) {
      ...
      if (w != NULL) {
        ...
        if (obj != NULL) {
          ...
          profiler->recordExternalSample(event._alloc_size, tid, LIVE_OBJECT,
                                         &event, call_trace_id);
        }
        jni->DeleteWeakGlobalRef(w);
      }

      if ((i % 32) == 31 || i == MAX_REFS - 1)
...
void ObjectSampler::GarbageCollectionStart(jvmtiEnv *jvmti) { live_refs.gc(); }
...
void ObjectSampler::recordAllocation(jvmtiEnv *jvmti, JNIEnv *jni,
  ...
  if (_live) {
    ...
    Profiler::instance()->recordSample(NULL, event._total_size, event_type,
                                       &event);
  }
}
