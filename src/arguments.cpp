...
//     resume           - start or resume profiling without resetting collected
//     data stop             - stop profiling dump             - dump collected
//     data without stopping profiling session check            - check if the
//     specified profiling event is available status           - print profiling
//     status (inactive / running for X seconds) meminfo          - print
//     profiler memory stats list             - show the list of available
//     profiling events version          - display the agent version event=EVENT
//     - which event to trace (cpu, wall, cache-misses, etc.) alloc[=BYTES]    -
//     profile allocations with BYTES interval live             - build
//     allocation profile from live objects only lock[=DURATION]  - profile
//     contended locks longer than DURATION ns wall[=NS]        - run wall clock
//     profiling together with CPU profiling collapsed        - dump collapsed
//     stacks (the format used by FlameGraph script) flamegraph       - produce
//     Flame Graph in HTML format tree             - produce call tree in HTML
//     format jfr              - dump events in Java Flight Recorder format
//     jfropts=OPTIONS  - JFR recording options: numeric bitmask or 'mem'
//     jfrsync[=CONFIG] - start Java Flight Recording with the given config
//     along with the profiler traces[=N]       - dump top N call traces
//     flat[=N]         - dump top N methods (aka flat profile)
//     samples          - count the number of samples (default)
//     total            - count the total value (time, bytes, etc.) instead of
//     samples chunksize=N      - approximate size of JFR chunk in bytes
//     (default: 100 MB) chunktime=N      - duration of JFR chunk in seconds
//     (default: 1 hour) timeout=TIME     - automatically stop profiler at TIME
//     (absolute or relative) loop=TIME        - run profiler in a loop
//     (continuous profiling) interval=N       - sampling interval in ns
//     (default: 10'000'000, i.e. 10 ms) jstackdepth=N    - maximum Java stack
//     depth (default: 2048) signal=N         - use alternative signal for cpu
//     or wall clock profiling features=LIST    - advanced stack trace features
//     (vtable, comptask)" safemode=BITS    - disable stack recovery techniques
//     (default: 0, i.e. everything enabled) file=FILENAME    - output file name
//     for dumping log=FILENAME     - log warnings and errors to the given
//     dedicated stream loglevel=LEVEL   - logging level: TRACE, DEBUG, INFO,
//     WARN, ERROR, or NONE server=ADDRESS   - start insecure HTTP server at
//     ADDRESS/PORT filter=FILTER    - thread filter threads          - profile
//     different threads separately sched            - group threads by
//     scheduling policy cstack=MODE      - how to collect C stack frames in
//     addition to Java stack
  ...
  for (char *arg = strtok(args_copy, ","); arg != NULL;
    ...
    SWITCH(arg) {
      ...
      if (value == NULL || (_signal = atoi(value)) <= 0) {
        ...
        // Two signals were specified: one for CPU profiling, another for wall
        // clock
      ...
      CASE("features")
      if (value != NULL) {
        if (strstr(value, "probesp"))
          _features.probe_sp = 1;
        if (strstr(value, "vtable"))
          _features.vtable_target = 1;
        if (strstr(value, "comptask"))
          _features.comp_task = 1;
      }

      CASE("safemode") {
        // Left for compatibility purpose; will be eventually migrated to
        // 'features'
      ...
      if (value == arg + 8)
        appendToEmbeddedList(_include, arg + 8);

      CASE("exclude")
      // Workaround -Wstringop-overflow warning
      if (value == arg + 8)
        appendToEmbeddedList(_exclude, arg + 8);
    ...
    if (c == '%') {
      ...
      if (c == 0) {
        ...
        ptr += snprintf(ptr, end - ptr, "%u",
                        max_files > 0 ? _file_num % max_files : _file_num);
        continue;
      } else if (c == '{') {
        char env_key[128];
...
int Arguments::parseTimeout(const char *str) {
  ...
  int ss = (p = strchr(p + 1, ':')) != NULL && p[1] >= '0' && p[1] <= '5'
               ? atoi(p + 1)
               : 0xff;
