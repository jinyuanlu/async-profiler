...
protected:
  ...
  static bool goodPtr(const void *ptr) {
    return (uintptr_t)ptr >= 0x1000 &&
           ((uintptr_t)ptr & (sizeof(uintptr_t) - 1)) == 0;
  }
...
public:
  ...
  static bool hasNativeThreadId() { return _has_native_thread_id; }
...
public:
  ...
  static VMKlass *fromOop(uintptr_t oop) {
    if (_narrow_klass_shift >= 0) {
      ...
      return (VMKlass *)(_narrow_klass_base +
                         (narrow_klass << _narrow_klass_shift));
    } else {
...
public:
  ...
  char *addr() { return *(char **)at(_flag_addr_offset); }

  char origin() {
    return _flag_origin_offset >= 0 ? (*(char *)at(_flag_origin_offset)) & 15
                                    : 0;
  }

  char get() { return *addr(); }

  void set(char value) { *addr() = value; }
};
