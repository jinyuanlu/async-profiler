...
enum EventMask { EM_CPU = 1, EM_ALLOC = 2, EM_LOCK = 4, EM_WALL = 8 };
...
static bool sortByCounter(const NamedMethodSample &a,
                          const NamedMethodSample &b) {
...
static inline int fastThreadId() {
  VMThread *vm_thread;
  if (VMStructs::hasNativeThreadId() &&
      (vm_thread = VMThread::current()) != NULL) {
...
*/
void Profiler::dumpJsonEvent(TraceEvent &event) {
  Arguments args;
  FrameName fn(args, args._style | STYLE_DOTTED, _epoch, _thread_names_lock,
               _thread_names);
  ...
  for (int i = 0; i < event.trace->trace->num_frames; i++) {
    ASGCT_CallFrame *frame_buffer = event.trace->trace->frames;
    ASGCT_CallFrame &frame = frame_buffer[i];
  ...
  json_event["frames"] = frames;

  std::string json_line = json_event.dump();

  json_line.push_back('\n');

  if (write(_out->get_fd(), json_line.c_str(), json_line.length()) < 0) {
...
int Profiler::getNativeTrace(void *ucontext, ASGCT_CallFrame *frames,
                             EventType event_type, int tid,
                             StackContext *java_ctx) {
  const void *callchain[MAX_NATIVE_FRAMES];
  int native_frames;

  if (_cstack == CSTACK_NO ||
      (event_type > EXECUTION_SAMPLE && _cstack == CSTACK_DEFAULT)) {
    return 0;
  }

  // Use PerfEvents stack walker for execution samples, or basic stack walker
  // for other events
  if (event_type == PERF_SAMPLE) {
    native_frames =
        PerfEvents::walk(tid, ucontext, callchain, MAX_NATIVE_FRAMES, java_ctx);
  } else if (_cstack == CSTACK_VM) {
    return 0;
  } else if (_cstack == CSTACK_DWARF) {
    native_frames = StackWalker::walkDwarf(ucontext, callchain,
                                           MAX_NATIVE_FRAMES, java_ctx);
  } else {
    native_frames =
        StackWalker::walkFP(ucontext, callchain, MAX_NATIVE_FRAMES, java_ctx);
  }
...
int Profiler::convertNativeTrace(int native_frames, const void **callchain,
  ...
  for (int i = 0; i < native_frames; i++) {
    ...
    if (current_method_name != NULL &&
        (mark = NativeFunc::mark(current_method_name)) != 0) {
      if (mark == MARK_INTERPRETER) {
        // This is C++ interpreter frame, this and later frames should be
        // reported as Java frames returned by AGCT. Terminate the scan here.
        return depth;
...
int Profiler::getJavaTraceAsync(void *ucontext, ASGCT_CallFrame *frames,
  ...
  if (_features.unwind_native && vm_thread->inJava()) {
    ...
    if (DWARF_SUPPORTED && java_ctx->sp != 0) {
      // If a thread is in Java state, unwind manually to the last known Java
      // frame, since JVM does not always correctly unwind native frames
...
int Profiler::getJavaTraceInternal(jvmtiFrameInfo *jvmti_frames,
  ...
  // https://github.com/async-profiler/async-profiler/issues/64
  JNIEnv *jni = VM::jni();
  if (jni == NULL) {
    return 0;
  }
...
u64 Profiler::recordSample(void *ucontext, u64 counter, EventType event_type,
  ...
  int num_frames = 0;
  if (_add_event_frame && event_type >= ALLOC_SAMPLE &&
      event_type <= PARK_SAMPLE) {
  ...
  num_frames +=
      getNativeTrace(ucontext, frames + num_frames, event_type, tid, &java_ctx);
  ...
  if (_cstack == CSTACK_VM) {
    ...
    // Async events
    int java_frames = getJavaTraceAsync(ucontext, frames + num_frames,
                                        _max_stack_depth, &java_ctx);
    if (java_frames > 0 && java_ctx.pc != NULL &&
        VMStructs::hasMethodStructs()) {
    ...
    if (VMStructs::_get_stack_trace != NULL) {
      // Object allocation in HotSpot happens at known places where it is safe
      // to call JVM TI, but not directly, since the thread is in_vm rather than
      // in_native
      num_frames += getJavaTraceInternal(jvmti_frames + num_frames,
                                         frames + num_frames, _max_stack_depth);
    } else {
      num_frames += getJavaTraceAsync(ucontext, frames + num_frames,
                                      _max_stack_depth, &java_ctx);
    }
  } else {
    // Lock events and instrumentation events can safely call synchronous JVM TI
    // stack walker. Skip Instrument.recordSample() method
    int start_depth = event_type == INSTRUMENTED_METHOD ? 1 : 0;
    num_frames +=
        getJavaTraceJvmti(jvmti_frames + num_frames, frames + num_frames,
  ...
  for (std::vector<CallTraceSample *>::const_iterator it = samples.begin();
       it != samples.end(); ++it) {
    CallTraceSample *sample = *it; // Dereference the iterator to get the sample
    TraceEvent ev = {timestamp, tid, sample};

    if (!_trace_events.push(ev)) {
      std::cerr << "Trace events overflow, dropping event: "
                << _trace_events.size() << std::endl;
    }
  }
...
void Profiler::startEventWriter() {
  ...
  if (pthread_create(&_event_writer_thread, NULL, eventWriterThreadEntry,
                     this) != 0) {
...
void Profiler::recordExternalSample(u64 counter, int tid, EventType event_type,
                                    Event *event, int num_frames,
                                    ASGCT_CallFrame *frames) {
  atomicInc(_total_samples);
  ...
  if (_add_sched_frame) {
    num_frames +=
        makeFrame(frames + num_frames, BCI_ERROR, OS::schedPolicy(tid));
  }
...
void Profiler::setupSignalHandlers() {
  ...
  if (prev_handler == AllocTracer::trapHandler) {
    ...
    return;
  } else if (prev_handler != (void *)SIG_DFL &&
             prev_handler != (void *)SIG_IGN) {
    orig_trapHandler = prev_handler;
  }

  if (VM::hotspot_version() > 0 || !VM::loaded()) {
    // HotSpot tolerates interposed SIGSEGV/SIGBUS handler; other JVMs probably
    // not
    ...
    for (int tid; (tid = thread_list->next()) != -1;) {
      ...
      if (it == _thread_names.end() || it->first != tid) {
        if (OS::threadName(tid, name_buf, sizeof(name_buf))) {
          _thread_names.insert(
              it, std::map<int, std::string>::value_type(tid, name_buf));
        }
      }
    }
...
Engine *Profiler::selectEngine(const char *event_name) {
  if (event_name == NULL) {
    ...
    return PerfEvents::supported() ? (Engine *)&perf_events
                                   : (Engine *)&wall_clock;
...
Error Profiler::start(Arguments &args, bool reset) {
  ...
  _event_mask =
      (args._event != NULL ? EM_CPU : 0) | (args._alloc >= 0 ? EM_ALLOC : 0) |
      (args._lock >= 0 ? EM_LOCK : 0) | (args._wall >= 0 ? EM_WALL : 0);
  if (_event_mask == 0) {
...
void Profiler::printUsedMemory(Writer &out) {
  ...
  snprintf(buf, sizeof(buf) - 1,
           ...
           "------------------------------\n"
           "             Total: %7zu KB\n",
           call_trace_storage / KB, flight_recording / KB, dictionaries / KB,
           code_cache / KB,
           (call_trace_storage + flight_recording + dictionaries + code_cache) /
               KB);
  out << buf;
}
...
void Profiler::dumpCollapsed(Writer &out, Arguments &args) {
  FrameName fn(args, args._style | STYLE_NO_SEMICOLON, _epoch,
  ...
  for (std::vector<CallTraceSample *>::const_iterator it = samples.begin();
    ...
    if (trace == NULL || excludeTrace(&fn, trace))
      continue;

    u64 counter =
        args._counter == COUNTER_SAMPLES ? (*it)->samples : (*it)->counter;
    if (counter == 0)
      continue;
...
void Profiler::dumpFlameGraph(Writer &out, Arguments &args, bool tree) {
  ...
  if (args._title == NULL) {
    ...
    if (args._counter == COUNTER_SAMPLES) {
      ...
      snprintf(title, sizeof(title), "%s (%s)", active_engine->title(),
               active_engine->units());
    }
  }

  FlameGraph flamegraph(args._title == NULL ? title : args._title,
                        args._counter, args._minwidth, args._reverse);

  {
    FrameName fn(args, args._style & ~STYLE_ANNOTATE, _epoch,
                 _thread_names_lock, _thread_names);
...
void Profiler::dumpText(Writer &out, Arguments &args) {
  ...
  u64 total_counter = 0;
  {
    ...
    for (std::map<u64, CallTraceSample>::const_iterator it = map.begin();
      ...
      if (trace == NULL || counter == 0)
        continue;

      total_counter += counter;
      if (trace->num_frames == 0 || excludeTrace(&fn, trace))
        continue;
  ...
  for (int i = 1; i < ASGCT_FAILURE_TYPES; i++) {
    ...
    if (err_string != NULL && _failures[i] > 0) {
      snprintf(buf, sizeof(buf), "%-20s: %lld (%.2f%%)\n", err_string,
               _failures[i], _failures[i] * spercent);
      out << buf;
    }
  }
  out << "\n";
  ...
  if (args._dump_traces > 0) {
    ...
    for (std::vector<CallTraceSample>::const_iterator it = samples.begin();
         it != samples.end() && --max_count >= 0; ++it) {
      snprintf(buf, sizeof(buf) - 1, "--- %lld %s (%.2f%%), %lld sample%s\n",
               it->counter, units_str, it->counter * cpercent, it->samples,
               it->samples == 1 ? "" : "s");
      out << buf;
  ...
  if (args._dump_flat > 0) {
    ...
    snprintf(buf, sizeof(buf) - 1,
             "%12s  percent  samples  top\n"
             "  ----------  -------  -------  ---\n",
             units_str);
    out << buf;

    int max_count = args._dump_flat;
    for (std::vector<NamedMethodSample>::const_iterator it = methods.begin();
         it != methods.end() && --max_count >= 0; ++it) {
...
void Profiler::startTimer() {
  if (VM::loaded()) {
    ...
    jmethodID init =
        jni->GetMethodID(Thread, "<init>", "(Ljava/lang/String;)V");
    ...
    if (name != NULL && init != NULL && setDaemon != NULL) {
      ...
      if (thread_obj != NULL) {
        ...
        if (VM::jvmti()->RunAgentThread(thread_obj, jvmtiTimerEntry, _timer_id,
                                        JVMTI_THREAD_NORM_PRIORITY) == 0) {
          return;
        }
        _timer_id = NULL;
      }
...
Error Profiler::runInternal(Arguments &args, Writer &out) {
  ...
  case ACTION_STOP: {
    ...
    if (args._output == OUTPUT_NONE) {
      ...
      out << "Profiling stopped after " << uptime()
          << " seconds. No dump options specified\n";
      break;
    }
    // Fall through
  }
...
Error Profiler::restart(Arguments &args) {
  ...
  if (args._file != NULL && args._output != OUTPUT_NONE &&
      args._output != OUTPUT_JFR) {
    FileWriter out(args.file());
    if (!out.is_open()) {
      return Error("Could not open output file");
    }
