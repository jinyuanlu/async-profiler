...
PerfEventType PerfEventType::AVAILABLE_EVENTS[] = {
    ...
    {"mem:breakpoint", BKPT_INTERVAL, PERF_TYPE_BREAKPOINT,
     0},                                              /* IDX_BREAKPOINT */
...
#warning "Compiling without LBR support. Kernel headers 4.1+ required"
  ...
  void *page = _use_mmap_page ? mmap(NULL, 2 * OS::page_size,
  ...
  int err;
  if (fcntl(fd, F_SETFL, O_ASYNC) < 0 || fcntl(fd, F_SETSIG, _signal) < 0 ||
      fcntl(fd, F_SETOWN_EX, &ex) < 0) {
    err = errno;
    Log::warn("perf_event fcntl failed: %s", strerror(err));
  } else if (ioctl(fd, PERF_EVENT_IOC_RESET, 0) < 0 ||
             ioctl(fd, PERF_EVENT_IOC_REFRESH, 1) < 0) {
    err = errno;
...
void PerfEvents::signalHandlerJ9(int signo, siginfo_t *siginfo,
                                 void *ucontext) {
  if (siginfo->si_code <= 0) {
    // Looks like an external signal; don't treat as a profiling event
    return;
  ...
  if (_enabled) {
    ...
    notif.num_frames = _cstack == CSTACK_NO
                           ? 0
                           : walk(OS::threadId(), ucontext, notif.addr,
                                  MAX_J9_NATIVE_FRAMES, &java_ctx);
...
const char *PerfEvents::title() {
  if (_event_type == NULL || _event_type->name == EVENT_CPU) {
    return "CPU profile";
  } else if (_event_type->type == PERF_TYPE_SOFTWARE ||
             _event_type->type == PERF_TYPE_HARDWARE ||
             _event_type->type == PERF_TYPE_HW_CACHE) {
...
Error PerfEvents::start(Arguments &args) {
  ...
  if (err) {
    ...
    if (err == EACCES || err == EPERM) {
      return Error("No access to perf events. Try --fdtransfer or --all-user "
                   "option or 'sysctl kernel.perf_event_paranoid=1'");
...
int PerfEvents::walk(int tid, void *ucontext, const void **callchain,
  ...
  if (_cstack == CSTACK_FP) {
    ...
    depth += StackWalker::walkDwarf(ucontext, callchain + depth,
                                    max_depth - depth, java_ctx);
  }
