...
static bool isCompilerEntry(const char *blob_name) {
  return strncmp(blob_name, "_ZN13CompileBroker25invoke_compiler_on_method",
                 45) == 0;
}
...
static void *resolveMethodIdEnd() { return NULL; }
...
bool VM::init(JavaVM *vm, bool attach) {
  ...
  Dl_info dl_info;
  if (dladdr((const void *)resolveMethodId, &dl_info) &&
      dl_info.dli_fname != NULL) {
    // Make sure async-profiler DSO cannot be unloaded, since it contains JVM
    // callbacks. Don't use ELF NODELETE flag because of
  ...
  _openj9 = !is_hotspot &&
            J9Ext::initialize(_jvmti, profiler->resolveSymbol("j9thread_self"));
  ...
  if (_can_sample_objects) {
    // SetHeapSamplingInterval does not have immediate effect, so apply the
    // configuration as early as possible to allow profiling all startup
    // allocations
...
extern "C" DLLEXPORT jint JNICALL Agent_OnLoad(JavaVM *vm, char *options,
                                               void *reserved) {
  if (!_global_args._preloaded) {
...
extern "C" DLLEXPORT jint JNICALL Agent_OnAttach(JavaVM *vm, char *options,
                                                 void *reserved) {
  Arguments args;
