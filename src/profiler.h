...
private:
  ...
  size_t cap() const { return sizeof(buf) / sizeof(*buf); }

  size_t inc_pos(size_t *pos) {
    size_t cur = *pos;
    *pos = (*pos + 1) % cap();
    return cur;
  }
...
public:
  TraceEventRingBuffer() : buf(), read_i(0), write_i(0), lock(), cv() {}
  ...
  TraceEvent poll() {
    std::unique_lock<std::mutex> lck(lock);
    cv.wait(lck, [this]() -> bool { return this->size() > 0; });
    if (size() > 0) {
      return buf[inc_pos(&read_i)];
    }
    return (TraceEvent){0, 0, nullptr};
  }
};
...
private:
  ...
  int getNativeTrace(void *ucontext, ASGCT_CallFrame *frames,
                     EventType event_type, int tid, StackContext *java_ctx);
  ...
  void dumpJsonEvent(TraceEvent &event);

  static Profiler *const _instance;

public:
  Profiler()
      : _state(NEW), _begin_trap(2), _end_trap(3), _thread_filter(),
        _call_trace_storage(), _jfr(), _start_time(0), _epoch(0), _gc_id(0),
        _timer_id(NULL), _max_stack_depth(0),
        _thread_events_state(JVMTI_DISABLE), _stubs_lock(),
        _runtime_stubs("[stubs]"), _native_libs(), _call_stub_begin(NULL),
        _call_stub_end(NULL), _dlopen_entry(NULL), _trace_events(),
        _event_writer_thread() {
    _out = new FileWriter("/tmp/java.trace");
  ...
  u64 recordSample(void *ucontext, u64 counter, EventType event_type,
                   Event *event);
  void recordExternalSample(u64 counter, int tid, EventType event_type,
