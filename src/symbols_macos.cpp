...
private:
  ...
  const section_64 *findSymbolPtrSection(const segment_command_64 *sc) {
    const section_64 *section =
        (const section_64 *)add(sc, sizeof(segment_command_64));
  ...
  void loadSymbols(const symtab_command *symtab, const char *text_base,
  ...
  void loadImports(const symtab_command *symtab,
    ...
    const uint32_t *isym =
        (const uint32_t *)add(link_base, dysymtab->indirectsymoff) +
        la_symbol_ptr->reserved1;
...
public:
  MachOParser(CodeCache *cc, const mach_header *image_base)
      : _cc(cc), _image_base(image_base) {}
    ...
    for (uint32_t i = 0; i < header->ncmds; i++) {
      if (lc->cmd == LC_SEGMENT_64) {
        ...
        if ((sc->initprot & 4) != 0) {
          ...
          if (link_base == UNDEFINED ||
              strcmp(sc->segname, "__LINKEDIT") == 0) {
        ...
        if (la_symbol_ptr != NULL && symtab != NULL && link_base != UNDEFINED) {
          loadImports(symtab, (const dysymtab_command *)lc, la_symbol_ptr,
                      link_base);
        }
      }
