...
INCLUDE_HELPER_CLASS(INSTRUMENT_NAME, INSTRUMENT_CLASS,
                     "one/profiler/Instrument")
...
public:
  ...
  u16 info() { return (u16)_info[0] << 8 | (u16)_info[1]; }
  ...
  bool equals(const char *value, u16 len) {
    return _tag == CONSTANT_Utf8 && info() == len &&
           memcmp(_info + 2, value, len) == 0;
  }

  bool matches(const char *value, u16 len) {
    if (len > 0 && value[len - 1] == '*') {
      return _tag == CONSTANT_Utf8 && info() >= len - 1 &&
             memcmp(_info + 2, value, len - 1) == 0;
    }
...
private:
  ...
  u8 get8() { return *get(1); }

  u16 get16() { return ntohs(*(u16 *)get(2)); }

  u32 get32() { return ntohl(*(u32 *)get(4)); }

  u64 get64() { return OS::ntoh64(*(u64 *)get(8)); }
  ...
  void put(const u8 *src, int bytes) { memcpy(alloc(bytes), src, bytes); }

  void put8(u8 v) { *alloc(1) = v; }

  void put16(u16 v) { *(u16 *)alloc(2) = htons(v); }

  void put32(u32 v) { *(u32 *)alloc(4) = htonl(v); }

  void put64(u64 v) { *(u64 *)alloc(8) = OS::hton64(v); }
...
public:
  BytecodeRewriter(const u8 *class_data, int class_data_len,
                   const char *target_class)
      : _src(class_data), _src_limit(class_data + class_data_len), _dst(NULL),
        _dst_len(0), _dst_capacity(class_data_len + 400), _cpool(NULL) {
  ...
  ~BytecodeRewriter() { delete[] _cpool; }
...
void BytecodeRewriter::rewriteMembers(Scope scope) {
  ...
  for (int i = 0; i < members_count; i++) {
    ...
    bool need_rewrite =
        ...
        (_target_signature == NULL ||
         _cpool[descriptor_index]->matches(_target_signature,
                                           _target_signature_len));
...
Error Instrument::check(Arguments &args) {
  if (!_instrument_class_loaded) {
    ...
    JNIEnv *jni = VM::jni();
    const JNINativeMethod native_method = {(char *)"recordSample",
                                           (char *)"()V", (void *)recordSample};

    jclass cls =
        jni->DefineClass(INSTRUMENT_NAME, NULL, (const jbyte *)INSTRUMENT_CLASS,
                         INCBIN_SIZEOF(INSTRUMENT_CLASS));
...
void Instrument::retransformMatchedClasses(jvmtiEnv *jvmti) {
  ...
  for (int i = 0; i < class_count; i++) {
    ...
    if (jvmti->GetClassSignature(classes[i], &signature, NULL) == 0) {
      if (signature[0] == 'L' &&
          strncmp(signature + 1, _target_class, len) == 0 &&
          signature[len + 1] == ';') {
...
void JNICALL Instrument::ClassFileLoadHook(
    ...
    const u8 *class_data, jint *new_class_data_len, u8 **new_class_data) {
...
void JNICALL Instrument::recordSample(JNIEnv *jni, jobject unused) {
  ...
  if (_interval <= 1 || ((atomicInc(_calls) + 1) % _interval) == 0) {
    ...
    Profiler::instance()->recordSample(NULL, _interval, INSTRUMENTED_METHOD,
                                       &event);
  }
}
