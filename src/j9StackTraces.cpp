...
enum { J9_STOPPED = 0x40, J9_HALT_THREAD_INSPECTION = 0x8000 };
...
public:
  ...
  void clearFlag(uintptr_t flag) { __sync_fetch_and_and(&_flags, ~flag); }
...
void J9StackTraces::timerLoop() {
  ...
  jvmtiFrameInfoExtended *jvmti_frames = (jvmtiFrameInfoExtended *)malloc(
      max_frames * sizeof(jvmtiFrameInfoExtended));
  ...
  while (true) {
    ...
    while (ptr < bytes) {
      J9StackTraceNotification *notif =
          (J9StackTraceNotification *)(notification_buf + ptr);
      u64 start_time = TSC::ticks();

      jthread thread = known_threads[notif->env];
      jint num_jvmti_frames;
      if (thread == NULL ||
          J9Ext::GetStackTraceExtended(thread, 0, _max_stack_depth,
                                       jvmti_frames, &num_jvmti_frames) != 0) {
      ...
      int num_frames = Profiler::instance()->convertNativeTrace(
          notif->num_frames, notif->addr, frames);

      for (int j = 0; j < num_jvmti_frames; j++) {
        frames[num_frames].method_id = jvmti_frames[j].method;
        frames[num_frames].bci =
            FrameType::encode(jvmti_frames[j].type, jvmti_frames[j].location);
        num_frames++;
      }
      ...
      Profiler::instance()->recordExternalSample(
          notif->counter, tid, EXECUTION_SAMPLE, &event, num_frames, frames);
