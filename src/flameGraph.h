...
public:
  ...
  Trie()
      : _children(), _total(0), _self(0), _inlined(0), _c1_compiled(0),
        _interpreted(0) {}
  ...
  u32 nameIndex(u32 key) const { return key & ((1 << 28) - 1); }
  ...
  int depth(u64 cutoff, u32 *name_order) const {
    int max_depth = 0;
    for (std::map<u32, Trie>::const_iterator it = _children.begin();
         it != _children.end(); ++it) {
...
private:
  ...
  void printTreeFrame(Writer &out, const Trie &f, int level,
                      const char **names);
...
public:
  FlameGraph(const char *title, Counter counter, double minwidth, bool reverse)
      : _root(), _cpool(), _title(title), _counter(counter),
        _minwidth(minwidth), _reverse(reverse), _last_level(0), _last_x(0),
        _last_total(0) {
    _buf[sizeof(_buf) - 1] = 0;
  }

  Trie *root() { return &_root; }
