...
public:
  Trap(int id)
      : _id(id), _unprotect(true), _protect(WX_MEMORY), _entry(0),
        _breakpoint_insn(BREAKPOINT) {}

  uintptr_t entry() { return _entry; }
  ...
  bool install() { return _entry == 0 || patch(_breakpoint_insn); }

  bool uninstall() { return _entry == 0 || patch(_saved_insn); }
