...
int FdTransferClient::requestPerfFd(int *tid, struct perf_event_attr *attr) {
  ...
  if (RESTARTABLE(send(_peer, &request, sizeof(request), 0)) !=
      sizeof(request)) {
  ...
  if (fd == -1) {
    ...
    // Update the TID of createForThread, in case the multiple threads' requests
    // got mixed up and we're now handling the response destined to another.
    // It's alright - the other thread(s) will finish the handling of our TID
    // perf fd.
    *tid = resp.tid;
  }
  return fd;
...
int FdTransferClient::recvFd(unsigned int type, struct fd_response *resp,
                             size_t resp_size) {
  struct msghdr msg = {0};
  ...
  if (resp->error == 0) {
    ...
    if (cmptr != NULL && cmptr->cmsg_len == CMSG_LEN(sizeof(newfd)) &&
      ...
      Log::warn("FdTransferClient recvmsg(): unexpected response with no "
                "SCM_RIGHTS: %s",
                strerror(errno));
      newfd = -1;
    }
