...
void J9ObjectSampler::JavaObjectAlloc(jvmtiEnv *jvmti, JNIEnv *jni,
...
void J9ObjectSampler::VMObjectAlloc(jvmtiEnv *jvmti, JNIEnv *jni,
                                    jthread thread, jobject object,
                                    jclass object_klass, jlong size) {
  if (_enabled && updateCounter(_allocated_bytes, size, _interval)) {
    recordAllocation(jvmti, jni, ALLOC_OUTSIDE_TLAB, object, object_klass,
                     size);
  }
}
...
Error J9ObjectSampler::start(Arguments &args) {
  ...
  if (jvmti->SetExtensionEventCallback(J9Ext::InstrumentableObjectAlloc_id,
                                       (jvmtiExtensionEvent)JavaObjectAlloc) !=
      0) {
...
void J9ObjectSampler::stop() {
  ...
  jvmti->SetEventNotificationMode(JVMTI_DISABLE,
                                  JVMTI_EVENT_GARBAGE_COLLECTION_START, NULL);
  jvmti->SetEventNotificationMode(JVMTI_DISABLE, JVMTI_EVENT_VM_OBJECT_ALLOC,
                                  NULL);
