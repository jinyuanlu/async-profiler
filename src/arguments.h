...
const char *const EVENT_NONE = "none";
...
enum SHORT_ENUM Counter { COUNTER_SAMPLES, COUNTER_TOTAL };

enum SHORT_ENUM Ring { RING_ANY, RING_KERNEL, RING_USER };
...
enum SHORT_ENUM Clock { CLK_DEFAULT, CLK_TSC, CLK_MONOTONIC };
...
enum JfrOption {
  ...
  JFR_SYNC_OPTS = NO_SYSTEM_INFO | NO_SYSTEM_PROPS | NO_NATIVE_LIBS |
                  NO_CPU_LOAD | NO_HEAP_SUMMARY
};
...
struct StackWalkFeatures {
  ...
  StackWalkFeatures()
      : unknown_java(1), unwind_stub(1), unwind_comp(1), unwind_native(1),
        java_anchor(1), gc_traces(1), probe_sp(0), vtable_target(0),
        comp_task(0), _reserved(0) {}
};
...
public:
  ...
  Arguments()
      : _buf(NULL), _shared(false), _action(ACTION_NONE),
        _counter(COUNTER_SAMPLES), _ring(RING_ANY), _event(NULL), _timeout(0),
        _interval(0), _alloc(-1), _lock(-1), _wall(-1),
        _jstackdepth(DEFAULT_JSTACKDEPTH), _signal(0), _file(NULL), _log(NULL),
        _loglevel(NULL), _unknown_arg(NULL), _server(NULL), _filter(NULL),
        _include(0), _exclude(0), _mcache(0), _loop(false), _preloaded(false),
        _threads(false), _sched(false), _live(false), _fdtransfer(false),
        _fdtransfer_path(NULL), _style(0), _features(), _cstack(CSTACK_DEFAULT),
        _clock(CLK_DEFAULT), _output(OUTPUT_NONE),
        _chunk_size(100 * 1024 * 1024), _chunk_time(3600), _jfr_sync(NULL),
        _jfr_options(0), _dump_traces(0), _dump_flat(0), _file_num(0),
        _begin(NULL), _end(NULL), _title(NULL), _minwidth(0), _reverse(false) {}
  ...
  bool hasOutputFile() const {
    return _file != NULL && (_action == ACTION_STOP || _action == ACTION_DUMP
                                 ? _output != OUTPUT_JFR
