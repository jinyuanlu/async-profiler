...
uintptr_t &StackFrame::retval() { return (uintptr_t &)REG(regs[0], x[0]); }

uintptr_t StackFrame::link() { return (uintptr_t)REG(regs[30], lr); }
...
uintptr_t StackFrame::method() { return (uintptr_t)REG(regs[12], x[12]); }

uintptr_t StackFrame::senderSP() { return (uintptr_t)REG(regs[19], x[19]); }
...
bool StackFrame::unwindStub(instruction_t *entry, const char *name,
  ...
  if (ip == entry || *ip == 0xd65f03c0 || strncmp(name, "itable", 6) == 0 ||
    ...
    // These are called from arraycopy stub that maintains the regular frame
    // link
...
void StackFrame::adjustSP(const void *entry, const void *pc, uintptr_t &sp) {
  ...
  if (ip > entry && (ip[-1] == 0xa9bf27ff ||
    ...
    // the stack, thus breaking the invariant: sender_sp == current_sp +
    // frame_size. Since JDK 21, there are more instructions between `blr` and
    // `add`, ignore them now for the sake of simplicity.
    //   stp  xzr, x9, [sp, #-16]!
    //   blr  x8
    //   ...
    //   add  sp, sp, #0x10
...
bool StackFrame::checkInterruptedSyscall() {
  ...
  if (retval() == (uintptr_t)-EINTR) {
    // Workaround for JDK-8237858: restart the interrupted poll / epoll_wait
    // manually
    ...
    if (nr == SYS_ppoll || (nr == SYS_epoll_pwait && (int)arg3() == -1)) {
      ...
      if ((pc() & 0xfff) < max_distance &&
          SafeAccess::load32((u32 *)(pc() - max_distance), 0) == 0) {
        return true;
      }
      // Try to restore the original value of x0 saved in another register
      for (uintptr_t prev_pc = pc() - 4; pc() - prev_pc <= max_distance;
           prev_pc -= 4) {
