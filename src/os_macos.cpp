...
public:
  ...
  void rewind() {
    if (_thread_array != NULL) {
      ...
      vm_deallocate(_task, (vm_address_t)_thread_array,
                    _thread_count * sizeof(thread_t));
      _thread_array = NULL;
    }
  }
...
JitWriteProtection::JitWriteProtection(bool enable) {
  ...
  if (*(volatile char *)0xfffffc10c) {
    u64 val =
        enable ? *(volatile u64 *)0xfffffc118 : *(volatile u64 *)0xfffffc110;
    u64 prev;
...
u64 OS::processStartTime() {
  ...
  if (start_time == 0) {
    ...
    if (proc_pidinfo(processId(), PROC_PIDTBSDINFO, 0, &info, sizeof(info)) >
        0) {
      start_time =
          (u64)info.pbi_start_tvsec * 1000 + info.pbi_start_tvusec / 1000;
    }
  }
...
u64 OS::hton64(u64 x) { return OSSwapHostToBigInt64(x); }

u64 OS::ntoh64(u64 x) { return OSSwapBigToHostInt64(x); }

int OS::getMaxThreadId() { return 0x7fffffff; }
...
bool OS::threadName(int thread_id, char *name_buf, size_t name_len) {
  ...
  return thread && pthread_getname_np(thread, name_buf, name_len) == 0 &&
         name_buf[0] != 0;
}
...
ThreadState OS::threadState(int thread_id) {
  ...
  if (thread_info((thread_act_t)thread_id, THREAD_BASIC_INFO,
                  (thread_info_t)&info, &size) != 0) {
    return THREAD_UNKNOWN;
  }
  return info.run_state == TH_STATE_RUNNING ? THREAD_RUNNING : THREAD_SLEEPING;
}

ThreadList *OS::listThreads() { return new MacThreadList(); }
...
int OS::getCpuCount() {
  ...
  return sysctlbyname("hw.logicalcpu", &cpu_count, &size, NULL, 0) == 0
             ? cpu_count
             : 1;
}
...
u64 OS::getTotalCpuTime(u64 *utime, u64 *stime) {
  ...
  kern_return_t ret =
      host_processor_info(host, PROCESSOR_CPU_LOAD_INFO, &cpu_count,
                          &cpu_info_array, &cpu_info_count);
  ...
  processor_cpu_load_info_data_t *cpu_load =
      (processor_cpu_load_info_data_t *)cpu_info_array;
  u64 user = 0;
  u64 system = 0;
  u64 idle = 0;
  ...
  vm_deallocate(mach_task_self(), (vm_address_t)cpu_info_array,
...
void OS::copyFile(int src_fd, int dst_fd, off_t offset, size_t size) {
  char *buf =
      (char *)mmap(NULL, size + offset, PROT_READ, MAP_PRIVATE, src_fd, 0);
  if (buf == NULL) {
    return;
  }
