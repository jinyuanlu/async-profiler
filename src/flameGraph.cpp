...
public:
  ...
  Node(u32 key, u32 order, const Trie &trie)
      : _key(key), _order(order), _trie(&trie) {}
...
Trie *FlameGraph::addChild(Trie *f, const char *name, FrameTypeId type,
                           u64 value) {
  size_t len = strlen(name);
  bool has_suffix = len > 4 && name[len - 4] == '_' && name[len - 3] == '[' &&
                    name[len - 1] == ']';
...
void FlameGraph::dump(Writer &out, bool tree) {
  ...
  _mintotal = _minwidth == 0 && tree ? _root._total / 1000
                                     : (u64)(_root._total * _minwidth / 100);
  ...
  if (tree) {
    ...
    for (std::map<std::string, u32>::const_iterator it = _cpool.begin();
         it != _cpool.end(); ++it) {
...
void FlameGraph::printFrame(Writer &out, u32 key, const Trie &f, int level,
                            u64 x) {
  ...
  if (f._total != _last_total || has_extra_types) {
    ...
    if (has_extra_types) {
      p += snprintf(p, 100, ",%llu,%llu,%llu", f._inlined, f._c1_compiled,
                    f._interpreted);
    }
  }
  ...
  for (std::map<u32, Trie>::const_iterator it = f._children.begin();
       it != f._children.end(); ++it) {
    children.push_back(
        Node(it->first, _name_order[f.nameIndex(it->first)], it->second));
  }
...
void FlameGraph::printTreeFrame(Writer &out, const Trie &f, int level,
                                const char **names) {
  std::vector<Node> children;
  children.reserve(f._children.size());
  for (std::map<u32, Trie>::const_iterator it = f._children.begin();
  ...
  for (size_t i = 0; i < children.size(); i++) {
    ...
    if (_reverse) {
      ...
      snprintf(_buf, sizeof(_buf) - 1,
               "<li><div%s>%.2f%% [%s] &#8226; self: %.2f%% [%s]</div> <span "
               "class=\"t%d\">%s</span>\n",
               div_class, trie->_total * pct, Format().thousands(trie->_total),
               trie->_self * pct, Format().thousands(trie->_self), type,
               name.c_str());
    }
    out << _buf;
...
const char *FlameGraph::printTill(Writer &out, const char *data,
                                  const char *till) {
