...
ThreadState OS::threadState(int thread_id) {
  ...
  if (read(fd, buf, sizeof(buf)) > 0) {
    char *s = strchr(buf, ')');
    state = s != NULL && (s[2] == 'R' || s[2] == 'D') ? THREAD_RUNNING
                                                      : THREAD_SLEEPING;
  }
...
int OS::getProfilingSignal(int mode) {
  ...
  const u64 allowed_signals = 1ULL << SIGPROF | 1ULL << SIGVTALRM |
                              1ULL << SIGSTKFLT | 1ULL << SIGPWR |
                              -(1ULL << SIGRTMIN);
...
int OS::getCpuCount() { return sysconf(_SC_NPROCESSORS_ONLN); }
