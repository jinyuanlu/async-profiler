...
JfrMetadata::JfrMetadata() : Element("root") {
  *this
      << (element("metadata")
          ...
          << type("boolean", T_BOOLEAN) << type("char", T_CHAR)
          << type("float", T_FLOAT) << type("double", T_DOUBLE)
          << type("byte", T_BYTE) << type("short", T_SHORT)
          << type("int", T_INT) << type("long", T_LONG)
          ...
          << (type("jdk.types.ThreadState", T_THREAD_STATE, "Java Thread State",
                   true)
          ...
          << (type("jdk.types.VirtualSpace", T_VIRTUAL_SPACE)
              ...
              << field("committedEnd", T_LONG, "Committed End Address",
                       F_ADDRESS)
          ...
          << (type("jdk.ExecutionSample", T_EXECUTION_SAMPLE,
          ...
          << (type("jdk.ObjectAllocationInNewTLAB", T_ALLOC_IN_NEW_TLAB,
          ...
          << (type("jdk.ObjectAllocationOutsideTLAB", T_ALLOC_OUTSIDE_TLAB,
          ...
          << (type("jdk.JavaMonitorEnter", T_MONITOR_ENTER,
              ...
              << field("previousOwner", T_THREAD, "Previous Monitor Owner",
                       F_CPOOL)
          ...
          << (type("jdk.ThreadPark", T_THREAD_PARK, "Java Thread Park")
              ...
              << field("address", T_LONG, "Address of Object Parked",
                       F_ADDRESS))
          ...
          << (type("jdk.ActiveRecording", T_ACTIVE_RECORDING,
                   "Async-profiler Recording")
              << category("Flight Recorder")
              << field("startTime", T_LONG, "Start Time", F_TIME_TICKS)
              << field("id", T_LONG, "Id") << field("name", T_STRING, "Name")
          ...
          << (type("jdk.ActiveSetting", T_ACTIVE_SETTING,
                   "Async-profiler Setting")
              << category("Flight Recorder")
              << field("startTime", T_LONG, "Start Time", F_TIME_TICKS)
          ...
          << (type("jdk.InitialSystemProperty", T_INITIAL_SYSTEM_PROPERTY,
          ...
          << (type("profiler.LiveObject", T_LIVE_OBJECT, "Live Object")
              ...
              << field("objectClass", T_CLASS, "Object Class", F_CPOOL)
              << field("allocationSize", T_LONG, "Allocation Size", F_BYTES)
              << field("allocationTime", T_LONG, "Allocation Time",
                       F_TIME_TICKS))

          << (type("jdk.jfr.Label", T_LABEL, NULL) << field("value", T_STRING))
      ...
      << element("region")
             .attribute("locale", "en_US")
             .attribute("gmtOffset", "0");
