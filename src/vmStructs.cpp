...
void VMStructs::initOffsets() {
  ...
  if (entry != 0 && stride != 0) {
    for (;; entry += stride) {
      ...
      if (strcmp(type, "Klass") == 0) {
      ...
      } else if (strcmp(type, "GrowableArrayBase") == 0 ||
                 strcmp(type, "GenericGrowableArray") == 0) {
        ...
        } else if (strcmp(field, "_flags") == 0 ||
                   strcmp(field, "origin") == 0) {
...
void VMStructs::resolveOffsets() {
  ...
  if (ccp != NULL && ccp->get() && _narrow_klass_base_addr != NULL &&
      _narrow_klass_shift_addr != NULL) {
  ...
  _has_class_names =
      ...
      (_compact_object_headers ? (_markword_klass_shift >= 0 &&
                                  _markword_monitor_value == MONITOR_BIT)
                               : _oop_klass_offset >= 0) &&
  ...
  _has_stack_structs =
      ...
      _code_offset != -1 && _data_offset >= 0 && _scopes_data_offset != -1 &&
      _scopes_pcs_offset >= 0 && _nmethod_metadata_offset >= 0 &&
      _thread_vframe_offset >= 0 && _thread_exception_offset >= 0 &&
      _constmethod_size >= 0;
  ...
  if (_code_heap_addr != NULL && _code_heap_low_addr != NULL &&
    ...
    unsigned int code_heap_count =
        *(unsigned int *)(code_heaps + _array_len_offset);
