...
static jmethodID getMethodId(VMMethod *method) {
  if (!inDeadZone(method) && aligned((uintptr_t)method)) {
    ...
    if (!inDeadZone(method_id) && aligned((uintptr_t)method_id) &&
        VMMethod::fromMethodID(method_id) == method) {
      return method_id;
    }
  }
  return NULL;
...
int StackWalker::walkVM(void *ucontext, ASGCT_CallFrame *frames,
                        int max_depth) {
  const void *pc;
  uintptr_t fp;
  uintptr_t sp;
  ...
  while (depth < max_depth) {
    if (CodeHeap::contains(pc)) {
      ...
      fillFrame(frames[depth++], BCI_NATIVE_FRAME,
                profiler->findNativeMethod(pc));
    }
