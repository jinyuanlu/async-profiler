...
enum GCWhen { BEFORE_GC, AFTER_GC };
...
static const char *const SETTING_CSTACK[] = {NULL,    "no",  "fp",
                                             "dwarf", "lbr", "vm"};
...
public:
  ...
  jint getLineNumber(jint bci) {
    ...
    int i = 1;
    while (i < _line_number_table_size &&
           bci >= _line_number_table[i].start_location) {
      i++;
    }
...
public:
  ...
  size_t usedMemory() {
    ...
    for (const_iterator it = begin(); it != end(); ++it) {
      ...
      bytes +=
          it->second._line_number_table_size * sizeof(jvmtiLineNumberEntry);
    }
    return bytes;
  }
};
...
private:
  ...
  bool fillJavaMethodInfo(MethodInfo *mi, jmethodID method, bool first_time) {
    ...
    if (first_time &&
        jvmti->GetLineNumberTable(method, &mi->_line_number_table_size,
...
public:
  ...
  u32 getSymbol(const char *name) { return _symbols.lookup(name); }
};
...
public:
  Recording(int fd, const char *master_recording_file, Arguments &args)
      : _fd(fd), _thread_set(), _method_map() {
    _master_recording_file =
        master_recording_file == NULL ? NULL : strdup(master_recording_file);
    ...
    _chunk_size = args._chunk_size <= 0
                      ? MAX_JLONG
    ...
    if (args.hasOption(IN_MEMORY) &&
        (_memfd = OS::createMemoryFile("async-profiler-recording")) >= 0) {
      _in_memory = true;
    }

    _cpu_monitor_enabled = !args.hasOption(NO_CPU_LOAD);
    if (_cpu_monitor_enabled) {
      _last_times.proc.real = OS::getProcessCpuTime(&_last_times.proc.user,
                                                    &_last_times.proc.system);
      _last_times.total.real = OS::getTotalCpuTime(&_last_times.total.user,
                                                   &_last_times.total.system);
    }

    _heap_monitor_enabled = !args.hasOption(NO_HEAP_SUMMARY) &&
                            VM::_totalMemory != NULL && VM::_freeMemory != NULL;
    _last_gc_id = 0;
  }
  ...
  void cpuMonitorCycle() {
    ...
    CpuTimes times;
    times.proc.real =
        OS::getProcessCpuTime(&times.proc.user, &times.proc.system);
    times.total.real =
    ...
    if (times.proc.real != (u64)-1 && times.proc.real > _last_times.proc.real) {
      ...
      proc_system =
          ratio((times.proc.system - _last_times.proc.system) / delta);
    }
  ...
  void heapMonitorCycle(u32 gc_id) {
    if (!_heap_monitor_enabled || gc_id == _last_gc_id)
      return;

    recordHeapSummary(&_monitor_buf, gc_id, AFTER_GC, VM::_totalMemory(),
                      VM::_freeMemory());
    flushIfNeeded(&_monitor_buf, SMALL_BUFFER_LIMIT);
  ...
  bool hasMasterRecording() const { return _master_recording_file != NULL; }
  ...
  Buffer *buffer(int lock_index) { return &_buf[lock_index]; }
  ...
  bool parseAgentProperties() {
    ...
    if (vm_support != NULL) {
      ...
      if (get_agent_props != NULL && to_string != NULL) {
        jobject props =
            env->CallStaticObjectMethod(vm_support, get_agent_props);
        if (props != NULL) {
  ...
  const char *getFeaturesString(char *str, size_t size, StackWalkFeatures f) {
    snprintf(str, size, "%s %s %s %s %s %s %s %s %s",
             ...
             f.gc_traces ? "gc_traces" : "-", f.probe_sp ? "probesp" : "-",
             f.vtable_target ? "vtable" : "-", f.comp_task ? "comptask" : "-");
    return str;
  }

  void flush(Buffer *buf) {
    ssize_t result =
        write(_in_memory ? _memfd : _fd, buf->data(), buf->offset());
    if (result > 0) {
  ...
  void writeMetadata(Buffer *buf) {
    ...
    buf->putVar32(0);
    buf->putVar32(0x7fffffff); // must not clash with JFR metadata ID, or 'jfr
                               // print' will break
  ...
  void writeSettings(Buffer *buf, Arguments &args) {
    ...
    writeStringSetting(buf, T_ACTIVE_RECORDING, "ring",
                       SETTING_RING[args._ring]);
    writeStringSetting(buf, T_ACTIVE_RECORDING, "cstack",
                       SETTING_CSTACK[args._cstack]);
    writeStringSetting(buf, T_ACTIVE_RECORDING, "clock",
                       SETTING_CLOCK[args._clock]);
    ...
    writeListSetting(buf, T_ACTIVE_RECORDING, "include", args._buf,
                     args._include);
    writeListSetting(buf, T_ACTIVE_RECORDING, "exclude", args._buf,
                     args._exclude);
    ...
    char str[256];
    writeStringSetting(buf, T_ACTIVE_RECORDING, "features",
                       getFeaturesString(str, sizeof(str), args._features));
    ...
    writeBoolSetting(buf, T_ACTIVE_RECORDING, "debugSymbols",
                     VM::loaded() && VMStructs::libjvm()->hasDebugSymbols());
  ...
  void writeBoolSetting(Buffer *buf, int category, const char *key,
                        bool value) {
  ...
  void writeJvmInfo(Buffer *buf) {
    if (_agent_properties == NULL &&
        !(VM::loaded() && parseAgentProperties())) {
      return;
    }
  ...
  void writeSymbols(Buffer *buf, Lookup *lookup) {
    ...
    for (std::map<u32, const char *>::const_iterator it = symbols.begin();
         it != symbols.end(); ++it) {
      flushIfNeeded(buf, RECORDING_BUFFER_LIMIT - MAX_STRING_LENGTH);
      buf->putVar64(it->first | _base_id);
  ...
  void recordAllocationInNewTLAB(Buffer *buf, int tid, u32 call_trace_id,
                                 AllocEvent *event) {
    int start = buf->skip(1);
  ...
  void recordAllocationOutsideTLAB(Buffer *buf, int tid, u32 call_trace_id,
                                   AllocEvent *event) {
    int start = buf->skip(1);
  ...
  void recordLiveObject(Buffer *buf, int tid, u32 call_trace_id,
                        LiveObject *event) {
    int start = buf->skip(1);
  ...
  void recordHeapSummary(Buffer *buf, u32 id, GCWhen when, u64 total_memory,
                         u64 free_memory) {
...
Error FlightRecorder::startMasterRecording(Arguments &args,
  ...
  if (_jfr_sync_class == NULL) {
    ...
    const JNINativeMethod native_method = {
        (char *)"stopProfiler", (char *)"()V", (void *)JfrSync_stopProfiler};

    jclass cls =
        env->DefineClass(JFR_SYNC_NAME, NULL, (const jbyte *)JFR_SYNC_CLASS,
                         INCBIN_SIZEOF(JFR_SYNC_CLASS));
    if (cls == NULL || env->RegisterNatives(cls, &native_method, 1) != 0 ||
        (_start_method = env->GetStaticMethodID(
             cls, "start", "(Ljava/lang/String;Ljava/lang/String;I)V")) ==
            NULL ||
        (_stop_method = env->GetStaticMethodID(cls, "stop", "()V")) == NULL ||
        (_box_method = env->GetStaticMethodID(
             cls, "box", "(I)Ljava/lang/Integer;")) == NULL ||
  ...
  if (options_class != NULL) {
    if (args._chunk_size > 0) {
      jmethodID method =
          env->GetStaticMethodID(options_class, "setMaxChunkSize", "(J)V");
      if (method != NULL) {
        env->CallStaticVoidMethod(
            options_class, method,
            args._chunk_size < 1024 * 1024 ? 1024 * 1024 : args._chunk_size);
      }
    }

    if (args._jstackdepth > 0) {
      jmethodID method = env->GetStaticMethodID(options_class, "setStackDepth",
                                                "(Ljava/lang/Integer;)V");
      if (method != NULL) {
        jobject value = env->CallStaticObjectMethod(
            _jfr_sync_class, _box_method, args._jstackdepth);
        if (value != NULL) {
  ...
  jobject jsettings =
      args._jfr_sync == NULL ? NULL : env->NewStringUTF(args._jfr_sync);
  int event_mask = (args._event != NULL ? 1 : 0) | (args._alloc >= 0 ? 2 : 0) |
                   (args._lock >= 0 ? 4 : 0) |
                   ((args._jfr_options ^ JFR_SYNC_OPTS) << 4);

  env->CallStaticVoidMethod(_jfr_sync_class, _start_method, jfilename,
                            jsettings, event_mask);
...
void FlightRecorder::recordEvent(int lock_index, int tid, u32 call_trace_id,
  ...
  if (_rec != NULL) {
    ...
    case INSTRUMENTED_METHOD:
      ...
      break;
    case ALLOC_SAMPLE:
      _rec->recordAllocationInNewTLAB(buf, tid, call_trace_id,
                                      (AllocEvent *)event);
      break;
    case ALLOC_OUTSIDE_TLAB:
      _rec->recordAllocationOutsideTLAB(buf, tid, call_trace_id,
                                        (AllocEvent *)event);
      break;
    case LIVE_OBJECT:
